...
- [x] 音效/粒子/小地圖/存檔
- [ ] 接到 Hugo 網站（projects.md + devlog）
- [ ] commit & push & draft PR (#1 https://github.com/darkbearlab/SlothMageGames/pull/1)

## 遊戲 2：《樹懶法師的自動化工房》Sloth Factory
路徑：`static/games/sloth-factory/`（單一 HTML 檔，內嵌全部程式）
網格式自動化生產遊戲（Factorio-lite）：挖礦機→輸送帶→熔爐→組裝機→研究。
- [ ] 完成並串進網站
