...
### 難度曲線的實測結論
- **空瓶只給 1 個不可行**：隨機盤面的可解率在 8 色時是 0%、12 色更慘，
  產生器會卡在重抽迴圈。空瓶固定 2 個，難度改往「顏色數」與「瓶深」加。
- 瓶深 5、6 很好用：可解率 98~100%、產生瞬間完成，而且解答步數從 37 拉到 58+
- 200 關連續產生：0 失敗、平均重抽 1.11 次、單關最久 468ms
